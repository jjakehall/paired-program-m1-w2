...
### Step One: Create the `com.techelevator.Department` class

Create a new class called `com.techelevator.Department.java` with the following requirements.
...
`com.techelevator.Department` must have one constructor that accepts two parameters: `departmentID` and `name`.
...
| email | String | x | x |
| salary | double | x | x |
| department | com.techelevator.Department | x | x |
| hireDate | String | x | x |
...
Create an instance variable in the `Application` class called `departments` to hold a `List<com.techelevator.Department>`.
...
> Tip: use the `com.techelevator.Department`s from the `departments` list to assign each employee's department. Retrieve the two departments you need by using the `get()` method.
...
In the `com.techelevator.Project` and `com.techelevator.Employee` classes, you used the type `String` for the dates. In a real-world application, if you needed to perform calculations on dates, you wouldn't use a `String`.
...
Create another `private` method in the `Application` class. Have the method accept a `String` and return a `com.techelevator.Department`. Give the method a descriptive name for what it does—for example, `getDepartmentByName`.

In the method, iterate through `departments`, and test if each department's `name` matches the `String` passed into the method. If it matches, return that `com.techelevator.Department`. If there's no match, return `null`.
